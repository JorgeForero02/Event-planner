...
const TipoNotificacion = db.TipoNotificacion;

const TipoNotificacionController = {
  getAll: async (req, res, next) => {
    try {
      const items = await TipoNotificacion.findAll();
  ...
  getById: async (req, res, next) => {
    try {
      const { id } = req.params;
      const item = await TipoNotificacion.findByPk(id);
  ...
  create: async (req, res, next) => {
    try {
      const newItem = await TipoNotificacion.create(req.body);
